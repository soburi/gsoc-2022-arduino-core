...
- **Notes:**
  - ArduinoCore-Zephyr defines a global GPIO numbering over GPIO controllers:
    - The global gpio number assigns sequential numbers to GPIO pins in the device tree.
    ...
    - All GPIO controllers are counted, even if not with `status = "okay"`.
